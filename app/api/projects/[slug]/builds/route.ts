...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const [job] = await prisma.$transaction([
      prisma.buildJob.create({ data: { projectId: project.id, prompt, parentId } }),
      prisma.chatMessage.create({ data: { conversation: { connectOrCreate: { where: { projectId: project.id }, create: { projectId: project.id } } }, role: 'user', content: prompt } }),
    ]);
