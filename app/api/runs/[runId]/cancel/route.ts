...
export async function POST(_: Request, { params }: { params: Promise<{ runId: string }> }) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    if (run.buildJobId) {
      const cancelled = await tx.buildJob.updateMany({ where: { id: run.buildJobId, status: { in: [BuildStatus.QUEUED, BuildStatus.RUNNING] } }, data: { status: BuildStatus.CANCELLED, finishedAt: new Date() } });
      // The worker may have completed after the initial read. Its transaction
      // locks this same job row; a completed job must keep its terminal run state.
      if (!cancelled.count) return null;
    } else {
      const cancelled = await tx.agentRun.updateMany({ where: { id: run.id, status: { in: ['QUEUED', 'PLANNING', 'AWAITING_APPROVAL', 'EXECUTING', 'VALIDATING'] } }, data: { status: AgentRunStatus.CANCELLED } });
      if (!cancelled.count) return null;
    }
  ...
  if (!updated) return NextResponse.json({ error: 'Run is already finished.' }, { status: 409 });
