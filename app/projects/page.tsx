...
export default async function ProjectsPage() {
  ...
  return <main className="projects-page"><header><div><p>AGENTIC CAD</p><h1>Your projects</h1><span>Design work, agent runs, revisions, and exports.</span></div><Link href="/projects/new"><Button>New project</Button></Link></header><section>{projects.length ? projects.map((project) => <Link href={`/projects/${project.slug}`} className="project-row" key={project.id}><div><strong>{project.title}</strong><span>{project.summary ?? 'No description yet'}</span></div><div>{project._count.revisions} revisions · {project.visibility.toLowerCase()}</div></Link>) : <div className="project-empty"><h2>Start a durable design workspace.</h2><p>Create a project, then every prompt becomes an editable revision.</p><Link href="/projects/new"><Button>Create project</Button></Link></div>}</section></main>;
}
