...
import { Box, CheckCircle2, Download, Eye, FileCode2, Hammer, Menu, MessageSquare, PanelLeft, Printer, Send, Wrench, X } from 'lucide-react';
...
function ArtifactPreview({ artifact }: { artifact: Artifact }) {
  ...
  useEffect(() => {
    if (!isText || isSvg) return;
    fetch(`/api/artifacts/${artifact.id}?inline=1`).then((response) => response.ok ? response.text() : '').then(setContent).catch(() => setContent('Preview unavailable.'));
...
export function ProjectWorkspace({ initialProject }: { initialProject: Project }) {
  ...
  const cancelSubagent = async (subagentId: string) => {
    if (!latestRun) return;
    const response = await fetch(`/api/runs/${latestRun.id}/subagents/${subagentId}/cancel`, { method: 'POST' });
    if (response.ok) await refreshProject();
  };

  const sendFollowUp = async (subagentId: string) => {
    if (!latestRun || !followUp[subagentId]?.trim()) return;
    const response = await fetch(`/api/runs/${latestRun.id}/subagents/${subagentId}/follow-up`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: followUp[subagentId] }) });
    if (response.ok) { setFollowUp((current) => ({ ...current, [subagentId]: '' })); await refreshProject(); }
  };
