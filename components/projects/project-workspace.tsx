...
export function ProjectWorkspace({ initialProject }: { initialProject: Project }) {
  ...
  useEffect(() => {
    if (!busy || !project.jobs[0]) return;
    const stream = new EventSource(`/api/builds/${project.jobs[0].id}/events`);
    stream.addEventListener('build', (message) => setEvents((current) => [...current, JSON.parse((message as MessageEvent).data)]));
    stream.addEventListener('complete', () => { stream.close(); setBusy(false); location.reload(); });
    return () => stream.close();
  }, [busy, project.jobs]);
  ...
  const build = async () => {
    ...
    setProject((current) => ({ ...current, jobs: [{ ...data.job, events: [] }, ...current.jobs] })); setEvents([]); setPrompt('');
  };
  ...
  const decidePlan = async (decision: 'approve' | 'reject') => {
    ...
    const data = await response.json();
    if (response.ok && decision === 'approve') { setBusy(true); setProject((current) => ({ ...current, jobs: [{ ...data.job, events: [] }, ...current.jobs] })); setEvents([]); }
    setPendingRun(null);
  };
  ...
  const editInChiliCad = async () => {
    ...
    window.localStorage.setItem(HANDOFF_TO_CHILI_STEP, await response.text());
