...
import { Bot, Box, CheckCircle2, FileCode2, Hammer, Send, Wrench } from 'lucide-react';
...
export function ProjectWorkspace({ initialProject }: { initialProject: Project }) {
  ...
  useEffect(() => { if (!busy || !project.jobs[0]) return; const stream = new EventSource(`/api/builds/${project.jobs[0].id}/events`); stream.addEventListener('build', (message) => setEvents((current) => [...current, JSON.parse((message as MessageEvent).data)])); stream.addEventListener('complete', () => { stream.close(); setBusy(false); location.reload(); }); return () => stream.close(); }, [busy, project.jobs]);
