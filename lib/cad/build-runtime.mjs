...
export async function buildFromCode(code, replicad) {
  ...
  // Preserve kernel precision; display rounding must not decide whether a part exists.
  const volume = built.reduce((sum, part) => sum + part.volume, 0);
  const surfaceArea = built.reduce((sum, part) => sum + part.surfaceArea, 0);
  const min = [Infinity, Infinity, Infinity];
  const max = [-Infinity, -Infinity, -Infinity];
  ...
  if (rawCount > 0) warnings.push(`${rawCount} raw mesh part${rawCount === 1 ? '' : 's'} included in STL and preview cannot be exported to STEP. STEP does not represent the complete model.`);
  const findings = [
  ...
  const valid = Number.isFinite(volume) && volume > 0 && Number.isFinite(surfaceArea) && surfaceArea > 0 && triangleCount > 0;
  const metrics = { volume, surfaceArea, partCount: built.length, triangleCount: Math.round(triangleCount), bounds: { min, max } };
  const validation = { valid, complete: valid && warnings.length === 0, warnings, score: valid ? Math.max(60, 100 - 10 * warnings.length) : 0, findings };
  // `parts` stays serializable (the kernel handle is stripped), so the live shapes are exposed
  // separately for consumers that project from them, such as drawing generation.
  return { metrics, validation, preview, stl: binaryStl(built), step, warnings, shapes: solidParts.map((part) => part.shape), parts: built.map(({ shape, ...part }) => part) };
}
