...
// Agentic CAD intentionally runs on one pinned model so generated geometry is reproducible across builds.
export const GEMINI_MODELS = ['gemini-3.8-flash'];
...
export async function generateCadCode({ prompt, previousCode, apiKey, onStage = () => {} }) {
  ...
  await onStage('draft', { model: first.model });
  ...
  if (!previousCode && !worthReviewing) {
    ...
    try {
      ...
      passes = 2;
      await onStage('fidelity', { model: second.model });
    } catch (error) {
