...
// A spec-like key (length, wingspan, wheelbase, ...) or a value with a number and a unit (e.g. "330 m")
// scores an infobox higher in pickInfobox below, but isn't required — most everyday objects (furniture,
// tools, household items) have a real, on-topic infobox (materials, designer, manufacturer, ...) without
// ever using a dimension field, and that's still useful grounding (better than nothing).
...
// Wikipedia's full-text search ranks on every term, so a leading descriptor ("wooden Adirondack
// chair", "a large steel gear") can bury the actual article under unrelated pages that happen to
// mention the descriptor prominently, or that only share a single generic noun ("chair", "gear")
// with a completely unrelated topic (a "bolt" search surfaces the sprinter Usain Bolt; a "gear"
// search surfaces the video game Metal Gear). Retry with the leading token dropped each time
// nothing relevant comes back, since English noun phrases put the head noun last — but always
// require at least two overlapping tokens to accept a match. A single generic-noun overlap is too
// weak a signal to trust at any query length: it's better to ground nothing than to ground wrong.
async function searchCandidates(query, promptTokens) {
    const scoreTitle = (title) => tokenize(title).filter((token) => token.length > 2 && promptTokens.includes(token)).length;
    const tokens = query.split(' ');
    for (let start = 0; start < tokens.length; start += 1) {
        const subQuery = tokens.slice(start).join(' ');
        const search = await api({ action: 'query', list: 'search', srsearch: subQuery, srlimit: '5', srprop: '' });
        const strong = (search?.query?.search ?? [])
            .map((result) => ({ title: result.title, score: scoreTitle(result.title) }))
            .filter((candidate) => candidate.score >= 2)
            .sort((a, b) => b.score - a.score);
        if (strong.length > 0)
            return strong.slice(0, MAX_CANDIDATES).map((candidate) => candidate.title);
    }
    return [];
}
export const _internals = { extractInfoboxes, pickInfobox, trimInfobox, cleanValue, tokenize, searchCandidates };
...
export async function getWikipediaReference(prompt, options = {}) {
    ...
    try {
        ...
        const candidates = await searchCandidates(query, promptTokens);
        // Try the most relevant candidate articles in order; keep the first with either a usable
        // infobox or (many everyday-object articles use plain prose, no infobox at all) a real
        // summary extract — either is better grounding than none.
        for (const title of candidates) {
            ...
            const chosen = pickInfobox(extractInfoboxes(wikitext), promptTokens, new Set(tokenize(title)));
            ...
            const value = {
                ...
                infobox: chosen ? trimInfobox(chosen) : [],
                image,
            };
